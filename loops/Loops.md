...
## while loop
- This loop is used when we only know the end condition so whenever its hit we can stop the loop. Here we use `condition` to make sure the loop stays running
```py
# syntax
while condition:
    pass
